...
def getJavaSentence(node, nodeType, reponame, max_len):
    ...
    def process_PaperChunk(node):
        return node.get('encoder_text', node.get('text', ''))
...
def getSentence(node, nodeType, reponame, max_len):
    if "fileName" in node or nodeType in {"Attribute", "Function", "Lambda", "PaperChunk"}:
        return getPythonSentence(node, nodeType, reponame, max_len)
    return getJavaSentence(node, nodeType, reponame, max_len)
