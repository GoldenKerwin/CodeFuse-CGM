...
def getRawGraph(filename, graph_dir="", suffix="json"):
    file_path = filename
    if graph_dir and not os.path.isabs(filename):
...
def collate_cgm(graph_dir, encoder, qa_type='mft', seq_l=8192, use_chat=True):
    def collate(batches):
        ...
        for batch in batches:
            result_batch = {}
            graph_path = batch.get('graph') or batch.get('repo')
            graph = getRawGraph(graph_path, graph_dir=graph_dir, suffix='json')

            if graph is not None:
                graph['reponame'] = os.path.basename(graph_path).split('.')[0]
                ...
                if qa_type == 'mft':
                    result_batch['task'] = task_to_id.get(batch.get('task', 'issue_fix'), task_to_id['issue_fix'])
            else:
                raise ValueError(f"graph none for {graph_path}")
...
def train(args):
    ...
    if args.peft:
        lm_task_type = getattr(TaskType, "CAUSAL_LM")
        enc_task_type_name = str(getattr(args, "enc_lora_task_type", "FEATURE_EXTRACTION")).upper()
        enc_task_type = getattr(TaskType, enc_task_type_name, TaskType.FEATURE_EXTRACTION)
        peft_config = LoraConfig(
            task_type=lm_task_type,
